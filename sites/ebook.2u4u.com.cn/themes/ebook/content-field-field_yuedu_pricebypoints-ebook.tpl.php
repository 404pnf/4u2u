 ...
 */
?>
<?php $flag = flag_get_flag('yuedu_freebie');
$limit_flag = flag_get_flag('limit_read_percentage');
if(!$limit_flag->is_flagged($node->nid) && !$flag->is_flagged($node->nid) ): ?>
    ...
    foreach ($items as $delta => $item) :
      if (!$item['empty']) : ?>
        <div class="field-item <?php print ($count % 2 ? 'odd' : 'even') ?>">
          ...
          <?php } ?>
  			<?php print $item['view'] ?>
  			<a href = "/yuedu/buy/<?=$node->nid?>">
  				<?php 
  					 $mpath = drupal_get_path('module', 'yuedu');
  					 $buy_image = $mpath.'/images/buy.png';
  					 echo "<image src=/$buy_image />"
  				?>
  			</a>
				<?php 

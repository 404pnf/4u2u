...
if (theme_get_setting('group_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('group_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'group') .'/css/tabs.css', 'theme', 'screen');
}
...
function group_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('group_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = group_id_safe('page-'. $path);
    $body_classes[] = group_id_safe('section-'. $section);
...
function group_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = group_id_safe('node-type-' . $vars['type']);
...
function group_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = group_id_safe('block-' . $vars['block']->module);
    $classes[] = group_id_safe('block-' . $vars['block']->region);
    $classes[] = group_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('group_block_editing') && user_access('administer blocks')) {
...
function group_preprocess_comment(&$vars, $hook) {
...
function group_menu_item_link($link) {
...
function group_menu_local_tasks() {
  $output = '';
...
function group_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = group_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function group_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb">'. implode(' » ', $breadcrumb) .'</div>';
  }
}
/* 老葛下面写的具有通用性,而且不会出现在聚合页面term掉错的情况
function group_print_zcxh($terms) {
$output=""; 
$new_array = array();
 ...
 */
function group_print_zcxh_by_object($terms) {
$output=""; 
$new_array = array();
foreach($terms as $tid=>$term ){
 $parent_terms = taxonomy_get_parents($tid);
...
function group_dl_sellprice($field){

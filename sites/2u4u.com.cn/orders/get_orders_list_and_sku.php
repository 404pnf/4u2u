...
$sql .= " AND uo.`order_id` in (3003,3075,3088,3095,3099,3122,3126,3126,3126,3128,3129,3135,3149,3149,3149,3149,3151,3155,3169,3179,3186,3188,3192,3195,3195,3195,3195,3195,3199)";
	  ...
	  if($_GET['info']=='update'){  
       ...
       if(strpos($content,";")){
        	...
        	foreach($book_list as $value){
        		...
        		if(strlen($value)>2){       		
		        	$value = trim($value);	        		       
		       		$new_query = "SELECT price,sku FROM `book_info` 
  ...
  if(strpos($new_value,"|")){
	    ...
	    if(substr($name_sql,-4)=='AND ')
	           $name_sql = substr($name_sql,0,-4);     		
                       ...
                       $new_sql = "SELECT price,sku FROM `book_info` 
	        		                    WHERE ".$name_sql."
	        		                  AND `sku` 
	        		                    IN (SELECT `sku` FROM `book_kc`)";

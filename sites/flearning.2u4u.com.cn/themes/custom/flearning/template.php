...
if (theme_get_setting('flearning_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('flearning_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'flearning') .'/css/tabs.css', 'theme', 'screen');
}
...
function flearning_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('flearning_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = flearning_id_safe('page-'. $path);
    $body_classes[] = flearning_id_safe('section-'. $section);
...
function flearning_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = flearning_id_safe('node-type-' . $vars['type']);
...
function flearning_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = flearning_id_safe('block-' . $vars['block']->module);
    $classes[] = flearning_id_safe('block-' . $vars['block']->region);
    $classes[] = flearning_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('flearning_block_editing') && user_access('administer blocks')) {
...
function flearning_preprocess_comment(&$vars, $hook) {
...
function flearning_menu_item_link($link) {
...
function flearning_menu_local_tasks() {
  $output = '';
...
function flearning_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = flearning_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function flearning_id_safe($string) {
...
function flearning_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb">'. implode(' » ', $breadcrumb) .'</div>';
  }
}
/* 老葛下面写的具有通用性,而且不会出现在聚合页面term掉错的情况
function flearning_print_zcxh($terms) {
$output=""; 
$new_array = array();
 ...
 */
function flearning_print_zcxh_by_object($terms) {
$output=""; 
$new_array = array();
foreach($terms as $tid=>$term ){
 $parent_terms = taxonomy_get_parents($tid);
...
function flearning_dl_sellprice($field){
...
function phptemplate_get_ebookterms($view_name,$display_id){
	$args=func_get_args();
	array_shift($args);	//remove $view_name
	if(count($args)){
		array_shift($args);	//remove $display_id
	...
	$output='';
	$ebook=xmlrpc('http://ebook.2u4u.com.cn/xmlrpc.php','views.get',$view_name,$display_id,$args,0,0,false);
	foreach($ebook as $etid=>$tem_terms ){
		$output.=$ebook[0][tid].',';
	}
	//$output.=$ebook[0];

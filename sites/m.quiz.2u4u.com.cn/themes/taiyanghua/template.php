...
// Add Zen Tabs styles
if (theme_get_setting('taiyanghua_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'taiyanghua') .'/css/tabs.css', 'theme', 'screen');
}
...
function taiyanghua_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('taiyanghua_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = taiyanghua_id_safe('page-'. $path);
    $body_classes[] = taiyanghua_id_safe('section-'. $section);
...
function taiyanghua_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = taiyanghua_id_safe('node-type-' . $vars['type']);
  $vars['classes'] = implode(' ', $classes); // Concatenate with spaces
}

function taiyanghua_preprocess_comment_wrapper(&$vars) {
  $classes = array();
...
function taiyanghua_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = taiyanghua_id_safe('block-' . $vars['block']->module);
    $classes[] = taiyanghua_id_safe('block-' . $vars['block']->region);
    $classes[] = taiyanghua_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('taiyanghua_block_editing') && user_access('administer blocks')) {
...
function taiyanghua_preprocess_comment(&$vars, $hook) {
...
function taiyanghua_menu_item_link($link) {
...
function taiyanghua_menu_local_tasks() {
  $output = '';
...
function taiyanghua_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = taiyanghua_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function taiyanghua_id_safe($string) {
...
*/
function taiyanghua_breadcrumb($breadcrumb) {
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('taiyanghua_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('taiyanghua_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('taiyanghua_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('taiyanghua_breadcrumb_title')) {
        if ($title = drupal_get_title()) {
          $trailing_separator = $breadcrumb_separator;
        }
      }
      elseif (theme_get_setting('taiyanghua_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }

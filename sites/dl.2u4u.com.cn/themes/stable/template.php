...
if (theme_get_setting('stable_rebuild_registry')) {
  drupal_rebuild_theme_registry();
}

// Add Zen Tabs styles
if (theme_get_setting('stable_zen_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'stable') .'/css/tabs.css', 'theme', 'screen');
}
...
function stable_preprocess_page(&$vars, $hook) {
  ...
  if (user_access('administer blocks')) {
	...
	if (theme_get_setting('stable_wireframe')) {
  ...
  if (!$vars['is_front']) {
    ...
    $body_classes[] = stable_id_safe('page-'. $path);
    $body_classes[] = stable_id_safe('section-'. $section);
...
function stable_preprocess_node(&$vars, $hook) {
  ...
  $classes[] = stable_id_safe('node-type-' . $vars['type']);
...
function stable_preprocess_block(&$vars, $hook) {
    $block = $vars['block'];

    // special block classes
    $classes = array('block');
    $classes[] = stable_id_safe('block-' . $vars['block']->module);
    $classes[] = stable_id_safe('block-' . $vars['block']->region);
    $classes[] = stable_id_safe('block-id-' . $vars['block']->bid);
    $classes[] = 'clearfix';
    ...
    if (theme_get_setting('stable_block_editing') && user_access('administer blocks')) {
...
function stable_preprocess_comment(&$vars, $hook) {
...
function stable_menu_item_link($link) {
...
function stable_menu_local_tasks() {
  $output = '';
...
function stable_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
...
#New line added to get unique classes for each menu item
  $css_class = stable_id_safe(str_replace(' ', '_', strip_tags($link)));
...
function stable_id_safe($string) {
...
function stable_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb">'. implode(' » ', $breadcrumb) .'</div>';
  }
}
/* 老葛下面写的具有通用性,而且不会出现在聚合页面term掉错的情况
function stable_print_zcxh($terms) {
$output=""; 
$new_array = array();
 ...
 */
function stable_print_zcxh_by_object($terms) {
$output=""; 
$new_array = array();
foreach($terms as $tid=>$term ){
 $parent_terms = taxonomy_get_parents($tid);
...
function stable_dl_sellprice($field){
...
function phptemplate_get_quizs($view_name,$display_id='default'){
	$args=func_get_args();
	...
	$output='';
	$quizs=xmlrpc('http://quiz.2u4u.com.cn/xmlrpc.php','views.get',$view_name,$display_id,$args,0,0,true);
	$output.=$quizs[0];
	
	return $output;
}

function phptemplate_get_group($view_name,$display_id='default'){
	$args=func_get_args();
	array_shift($args);	//remove $view_name
	if(count($args)){
		array_shift($args);	//remove $display_id
	...
	$output='';
	$group=xmlrpc('http://group.2u4u.com.cn/xmlrpc.php','views.get',$view_name,$display_id,$args,0,0,true);
	$output.=$group[0];

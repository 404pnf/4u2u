...
function highlight(findsubtitles,id){
	...
	if(top_val>50)
		$(document).scrollTop(top_val-50);	
	//alert(scrolltop+' time_id:'+id+' top_val:'+top_val);
	if(navigator.platform == 'iPad' || navigator.platform == 'iPhone' || navigator.platform == 'iPod')
	{
		findsubtitles.parents('.smart_blog_subtitle').prev('.smart_blog_audio').children('.jp-audio').attr('style','position:absolute;top:'+(scrolltop+10)+'px;left:20px;');
	};
		...
		$('.smart_blog_subtitle').each(function(){
			...
			*/
			...
			if(scrolltop >= (audiotop-9) && scrolltop >= (zimuTop-74) && scrolltop <=(zimuTop+zimuHeight)){
				...
				if(navigator.platform == 'iPad' || navigator.platform == 'iPhone' || navigator.platform == 'iPod')
				{
					$(this).prev('.smart_blog_audio').children('.jp-audio').attr('style','position:absolute;top:'+(scrolltop+10)+'px;left:20px;');
				}
			}	
